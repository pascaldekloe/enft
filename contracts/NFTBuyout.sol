...
function offer(address target, Conditions calldata c) public payable {
	...
	require(ERC165(target).supportsInterface(type(ERC721).interfaceId), "need standard NTF");

	// fail-fast: trade requires allowance to this contract
	require(ERC20(c.token).allowance(msg.sender, address(this)) != 0, "no payment allowance");

	buyouts[target][msg.sender] = c;
	emit NFTBuyoutOffer(target, msg.sender);
}

/// @notice Each token is subject to a dedicated trade amount.
/// @dev ⚠️ Newly minted tokens may alter expectations.
/// @param target The ERC-721 contract
/// @param tokenID The NFT in subject
/// @return amount The ERC-20 quantity
/// @return token The ERC-20 contract
function tokenPrice(address target, uint256 tokenID, address buyer) public view returns (uint256 amount, address token) {
	Conditions memory c = buyouts[target][buyer];
	return (tokenPriceFromConditions(c, tokenID), c.token);
}
...
/// @param target The ERC-721 contract
/// @param tokenID The NTF in subject
/// @param buyer The acquisition party
/// @param wantAmount The minimal price expectation (to prevent races)
/// @param wantToken The ERC-20 unit expectation (to prevent races)
function redeemToken(address target, uint256 tokenID, address buyer, uint256 wantAmount, address wantToken) public {
	require(msg.sender != buyer, "sell to self");

	Conditions memory c = buyouts[target][buyer];
	uint256 amount = tokenPriceFromConditions(c, tokenID);
	require(amount >= wantAmount, "trade amount mismatch");
	require(c.token == wantToken, "trade token mismatch");

	ERC721(target).transferFrom(msg.sender, buyer, tokenID);
	ERC20(c.token).transferFrom(buyer, msg.sender, amount);
}
